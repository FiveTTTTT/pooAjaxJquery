...
$requete = $bdd->query('SELECT * FROM messages ORDER BY id DESC LIMIT 0,5');
$mm = "";
...
while ($donnees = $requete->fetch()) {
    ...
    if ($donnees['user_name'] == $_POST['pseudo']) {
        $cl = "maCl";
        $mm =  "<p class='" . $cl . "' >" . $donnees['message'] . "</p>" . $mm;
    } else {
        $cl = "othCl";
        $mm =  "<p class='" . $cl . "' >" . $donnees['user_name'] . " : " . $donnees['message'] . "</p>" . $mm;
    }
}
